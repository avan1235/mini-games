...
object UserService {
  ...
  suspend fun sendConfirmationEmail(userEntity: UserEntity, host: String) {
    if (!REQUIRE_EMAIL_VERIFY) return
    val confirmUrl = createConfirmUrl(userEntity, host)
    EmailService.send(
      userEntity.email,
  ...
  private fun createConfirmUrl(userEntity: UserEntity, host: String): String =
    "https://$host/${USER_CONFIRM_GET(userEntity.confirmHash)}"
